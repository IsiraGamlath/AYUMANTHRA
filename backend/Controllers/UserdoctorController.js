...
// Get all doctors
// Update the getAllDoctors function
const getAllDoctors = async (req, res) => {
  try {
    const { mode } = req.query; // Get mode from query parameter
    
    const filter = mode ? { mode: mode } : {}; // Filter if mode provided
...
const addDoctor = async (req, res) => {
  ...
  try {
    ...
    const doctor = new Doctor({
      ...
      available,
      mode, // ✅ Include mode
      doctorFee: doctorFee || 50, // ✅ Include doctor fee with default
    });
