...
function AddRoutine() {
  ...
  const handleDurationKeyDown = (e) => {
    // Allow letters, numbers, space, backspace, delete, arrow keys, home, end
    const allowedKeys = [
      'Backspace', 'Delete', 'Tab', 'Escape', 'Enter',
      'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown',
      'Home', 'End'
  ...
  const handleArrayInputKeyDown = (e) => {
    // Allow most characters for array inputs
    const allowedKeys = [
      'Backspace', 'Delete', 'Tab', 'Escape', 'Enter',
      'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown',
      'Home', 'End'
    ...
    // Allow letters, numbers, space, comma, hyphen, underscore, period
    const regex = /^[a-zA-Z0-9 ,\-_.]$/;
    if (!regex.test(e.key)) {
      e.preventDefault();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isAuthenticated || !user) {
      alert("Please log in to create a routine.");
      navigate("/loginH");
      return;
    }
  ...
  const sendRequest = async () => {
    ...
    const requestData = {
      ...
      // Add userId for user-specific routines
      userId: user?._id,
    };
