...
const DupdateUser = ({ doctor, onClose, onDoctorUpdated }) => {
  ...
  useEffect(() => {
    setInputs({
      ...doctor,
      doctorFee: doctor.doctorFee || 50 // Ensure doctorFee is always set
    });
  }, [doctor]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updateData = {
        ...
        doctorFee: Number(inputs.doctorFee || 50),
      };

      console.log('Updating doctor with data:', updateData);

      const response = await axios.put(`http://localhost:5000/doctors/${doctor._id}`, updateData);

      console.log('Update response:', response.data);
      
      // Force refresh the doctor list
      onDoctorUpdated();
      onClose();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-2 text-sm">
          ...
          <div>
            <input
              type="number"
              name="doctorFee"
              value={inputs.doctorFee || ''}
              onChange={handleChange}
              placeholder="Doctor Fee (LKR)"
              min="10"
