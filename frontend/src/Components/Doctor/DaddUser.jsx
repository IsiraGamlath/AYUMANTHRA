...
const DaddUser = ({ onClose, onDoctorAdded }) => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "doctorFee":
        if (value === "") error = "Doctor fee is required.";
        else if (Number(value) < 0)
          error = "Doctor fee cannot be a negative number.";
        else if (Number(value) < 10)
          error = "Doctor fee must be at least 10.";
        break;
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border ${
                errors.doctorFee ? "border-red-500" : "border-gray-300"
              } rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm`}
            />
            {errors.doctorFee && (
              <p className="text-red-500 text-xs mt-1">
