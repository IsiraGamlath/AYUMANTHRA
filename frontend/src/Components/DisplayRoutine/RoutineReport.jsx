  ...
  useEffect(() => {
    const fetchRoutine = async () => {
      try {
        ...
        // Fetch dosha recommendations if dosha is available
        if (routineData && routineData.dosha) {
          fetchDoshaRecommendations(routineData.dosha);
        }
      } catch (error) {
  ...
  const fetchDoshaRecommendations = async (dosha) => {
    ...
    try {
      ...
      if (response.data && response.data.recommendations) {
        ...
        console.log("Fetched dosha recommendations:", response.data.recommendations);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="no-print mb-8">
          ...
          {/* Backend Dosha Recommendations Section */}
          {loadingDosha && (
            <div className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-green-500 mx-auto mb-4"></div>
              <p className="text-green-600 font-medium">Loading dosha recommendations from backend...</p>
            </div>
          )}

          {!loadingDosha && doshaRecommendations && (
            <div className="p-8 bg-gradient-to-r from-green-50 to-blue-50 border-b border-gray-200">
              <h3 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
                <Leaf className="w-6 h-6 mr-3 text-green-500" />
                Ayurvedic Wisdom for {routine.dosha} Dosha (From Backend)
              </h3>
              <p className="text-sm text-gray-600 mb-6 italic">
                ✨ These personalized recommendations are based on ancient Ayurvedic principles for {routine.dosha} constitution
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Object.entries(doshaRecommendations).map(([key, items]) => {
                  ...
                  return (
                    <div key={key} className="bg-white rounded-lg p-5 shadow-sm border border-green-200">
                      <h4 className="font-semibold text-green-700 mb-3 text-base">{label}</h4>
                      <ul className="space-y-2">
                        {items.map((item, index) => (
              ...
              <div className="mt-6 text-center">
                <p className="text-xs text-gray-500 italic">
                  These recommendations are dynamically fetched from our Ayurvedic knowledge base
                </p>
              </div>
            </div>
          )}

          {/* Ayurvedic Recommendations - Stored in Database */}
          <div className="p-8">
            <div className="mb-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
                <Target className="w-6 h-6 mr-3 text-blue-500" />
                Your Saved {routine.dosha} Dosha Recommendations
              </h3>
              <p className="text-sm text-gray-600 mb-6">
                These are your personalized selections saved in your routine
              </p>
