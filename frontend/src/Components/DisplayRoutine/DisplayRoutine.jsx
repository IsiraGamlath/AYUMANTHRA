...
function DisplayRoutine() {
  ...
  const [doshaRecommendations, setDoshaRecommendations] = useState({});
  const [loadingDosha, setLoadingDosha] = useState({});
  
  // ADD THIS: Get user from auth context
  ...
  const fetchHandler = async () => {
    try {
      ...
      console.log("User email:", user?.userGmail || user?.email);
      
      if (isAuthenticated && user) {
        // Use user-specific endpoint
        const userEmail = user.userGmail || user.email;
        console.log("Using user email:", userEmail);
        const response = await axios.get(`${API_URL}/user-email/${userEmail}`);
        console.log("Fetched user routines:", response.data);
        setRoutines(response.data || []);
      } else {
        // Fallback to all routines if not authenticated
        const response = await axios.get(API_URL);
        console.log("Fetched all routines:", response.data);
        setRoutines(response.data || []);
      }
  ...
  // Fetch dosha recommendations for a specific routine
  const fetchDoshaRecommendations = async (dosha, routineId) => {
    if (!dosha || doshaRecommendations[routineId]) return;
    
    setLoadingDosha(prev => ({ ...prev, [routineId]: true }));
    
    try {
      const response = await axios.get(`http://localhost:5000/routines/dosha/${dosha}`);
      
      if (response.data && response.data.recommendations) {
        setDoshaRecommendations(prev => ({
          ...prev,
          [routineId]: response.data.recommendations
        }));
      }
    } catch (error) {
      console.error("Failed to fetch dosha recommendations:", error);
    } finally {
      setLoadingDosha(prev => ({ ...prev, [routineId]: false }));
    }
  };

  useEffect(() => {
    fetchHandler();
  }, [isAuthenticated, user]); // ADD user and isAuthenticated to dependencies

  // Toggle expanded view for routine details
  const toggleExpanded = (routineId, dosha) => {
    const isExpanding = !expandedRoutines[routineId];
    
    setExpandedRoutines(prev => ({
      ...prev,
      [routineId]: isExpanding
    }));

    // Fetch dosha recommendations when expanding
    if (isExpanding && dosha) {
      fetchDoshaRecommendations(dosha, routineId);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {/* ADD THIS: Show user info */}
          {isAuthenticated && user && (
            <p className="text-gray-600 mt-2">
              Welcome back, <span className="font-semibold text-green-700">{user.userName || user.email}</span>
            </p>
          )}
        </div>
        ...
        {filteredRoutines.length === 0 && !loading && !error ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              {searchTerm || filterDosha || filterDifficulty || filterTimeOfDay
                ...
                : "No routines available at the moment. Create your first wellness routine!"}
            </p>
            <Link
              to="/add-routine"
          ...
          <div className="space-y-6">
            {filteredRoutines.map((routine) => {
              ...
              const routineDosha = doshaRecommendations[routine._id];
              const isLoadingDoshaData = loadingDosha[routine._id];
              ...
              return (
                <div key={routine._id} className="bg-white rounded-xl shadow-lg border-2 border-green-200 overflow-hidden">
                  ...
                  <div className="p-6 border-b border-green-100">
                    ...
                    {/* Toggle Button */}
                    <button
                      onClick={() => toggleExpanded(routine._id, routine.dosha)}
                  ...
                  {expandedRoutines[routine._id] && (
                    <div className="p-6 bg-gradient-to-br from-green-25 to-blue-25">
                      <h3 className="text-lg font-semibold text-green-800 mb-6 flex items-center">
                        ...
                        {routine.dosha || "Unknown"} Dosha Recommendations from Backend
                      </h3>

                      {/* Loading State for Dosha Data */}
                      {isLoadingDoshaData && (
                        <div className="text-center py-8">
                          <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-green-500 mx-auto mb-4"></div>
                          <p className="text-green-600 font-medium">Loading dosha recommendations...</p>
                        </div>
                      )}

                      {/* Backend Dosha Recommendations */}
                      {!isLoadingDoshaData && routineDosha && (
                        <div className="mb-8 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-6 border-2 border-green-200">
                          <h4 className="text-md font-semibold text-green-800 mb-4 flex items-center">
                            ...
                            Ayurvedic Recommendations for {routine.dosha} Dosha
                          </h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {Object.entries(routineDosha).map(([key, items]) => {
                              const labels = {
                                diet: "🥗 Diet",
                                herbs: "🌱 Herbs",
                                yoga: "🧘 Yoga",
                              ...
                              return (
                                <div key={key} className="bg-white rounded-lg p-4 shadow-sm border border-green-100">
                                  <h5 className="font-semibold text-green-700 mb-2">{label}</h5>
                          ...
                          <p className="text-sm text-green-600 mt-4 text-center italic">
                            ✨ These are personalized recommendations based on {routine.dosha} dosha characteristics
                          </p>
                        </div>
                      )}

                      {/* Stored Routine Data from Database */}
                      <h4 className="text-md font-semibold text-blue-800 mb-4 flex items-center">
                        <FileText className="w-5 h-5 mr-2" />
                        Your Saved Routine Details
                      </h4>
